...
class PlumeHeightPredictor:
    def __init__(self, data: pd.DataFrame):
        """
        Initialize the PlumeHeightPredictor class for Bayesian Regression.
    ...
    def calculate_percentiles_with_uncertainty(self, x: Union[float, np.ndarray], percentiles: List[int] = [5, 25, 50, 75, 95], num_samples: int = 10000) -> pd.DataFrame:
        ...
        for p in percentiles:
            ...
            # Calculate Best_Height_Estimate_km from the 95th and 5th percentiles
        if 'HeightPercentile_95' in results and 'HeightPercentile_5' in results:
            results['Best_Height_Estimate_km'] = (results['HeightPercentile_95'] - results['HeightPercentile_5'])
            
            
            # Calculate Best_Height_Estimate_km_Uncertainty
        if 'Uncertainty_Upper_95' in results and 'Uncertainty_Upper_5' in results:
            results['Best_Height_Estimate_km_Uncertainty'] = (
    ...
    def calculate_percentiles(self, x: Union[float, np.ndarray], percentiles: List[int] = [5, 25, 50, 75, 95]) -> pd.DataFrame:
        ...
        results = pd.DataFrame({'MER_kg/s': x})
        for p in percentiles:
            results[f'HeightPercentile_{p}'] = 10**(y_mean + t.ppf(p / 100, df=len(self.y) - 2) * np.sqrt(self.sigma2))

        return results

    def predict_height_with_uncertainty(self, mer_values: Union[float, List[float]], output_file: str = 'predicted_height_with_uncertainty.csv') -> pd.DataFrame:
        """
        Predict Height values from single or multiple MER values and include uncertainty estimates.
    ...
    def plot_percentiles_with_uncertainty(self, x_vals: np.ndarray, percentiles: List[int] = [5, 25, 50, 75, 95]):
        """
        Plot percentiles with uncertainty ribbons vs MER_kg/s.
        ...
        plt.yscale('log')
        plt.xlabel('MER_kg/s')
        plt.ylabel('Height_km.a.v.l')
        plt.title('Percentiles vs MER_kg/s with Uncertainty')
        plt.legend()
        plt.show()

    def plot_percentiles_vs_mer(self):
        """
        Plot percentiles vs MER_kg/s with predicted percentile values in the colorbar.
        """
        ...
        for p in percentiles:
            plt.plot(results['MER_kg/s'], results[f'HeightPercentile_{p}'], 
                    label=f'{p}th Percentile', linewidth=2.5)

        plt.xscale('log')
        plt.xlabel('Mass Eruption Rate (MER) [kg/s]')
        plt.ylabel('Height (km a.v.l)')
        plt.title('Percentiles of Height vs MER')
        ...
        mer = np.log10(results['MER_kg/s'])  # Log scale for MER
        mer=results['MER_kg/s']
        
        heights = np.array([results[f'HeightPercentile_{p}'] for p in percentiles])

        # Create 2D grid
        X, Y = np.meshgrid(mer, percentiles)  # X for MER, Y for percentiles
        Z = heights  # Z values are heights
        ...
        plt.xlabel('Log10(MER) (kg/s)', weight='bold')
        plt.ylabel('Percentiles', weight='bold')
        plt.title('2D Filled Contour Plot: MER vs Percentiles', weight='bold')
        plt.grid(alpha=0.5)
        plt.tight_layout()
        plt.show()
